...
def run_takeoff_agent(
    ...
    for ep in range(episodes):
        ...
        print(f"[TAKEOFF] Episode {ep+1}/{episodes} | Return: {ep_return:.1f} | Frames: {len(episode_frames)}")

    env.close()
    
    # Save frames to HDF5 format
    h5_file = video_path / "takeoff_video.h5"
    ...
    print(f"Video saved to: {h5_file.absolute()}")
